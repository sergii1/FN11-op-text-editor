...
TextEditor::TextEditor(QWidget* pwgt){
    ...
    pmnuEdit->addAction("Копировать", this, SLOT(slotCopy()));
    pmnuEdit->addAction("Вставить",this, SLOT(slotPaste()));
    pmnuEdit->addAction("Удалить", this, SLOT(slotDelete()));
...
void TextEditor::slotOpen()
{
    ...
    if (file.open(QIODevice::ReadOnly)){
        ...
        (new QErrorMessage(this))->showMessage("Ошибка открытия файла ");
    }
    TE->setText(Text);
    QFileInfo fInfo(file);
    m_tbw->addTab(TE, fInfo.baseName() + "." + fInfo.completeSuffix());
    //эквивалентно вызову this->setWindowTitle. Метод setWindowTitle определен в базовом классе
    //для textEditor - QMainWindow
...
void TextEditor::slotSave()
{
    ...
    //m_tbw->tabText(0) - текущее название файла
    //m_tbw->currentIndex()
...
//срабатывает на перетаскивание документа
void TextEditor::dropEvent(QDropEvent *pe)
{
    ...
    foreach(QUrl url, urlList) {
        ...
        m_StringList.push_back(fInfo.baseName());
        m_StringListModel->setStringList(m_StringList);
        qDebug()<<"ext = "<< fInfo.completeSuffix();
        new Highlighter("Syntax.xml", fInfo.completeSuffix(), TE->document());
        m_VectorFlagsChanged.push_back(false);
        file.close();
        connect(TE, SIGNAL(textChanged()), this, SLOT(slotChanged()));
